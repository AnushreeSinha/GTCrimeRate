...
function init(){
    ...
    var heatmap = new OpenLayers.Layer.HeatCanvas("Heat Canvas", map, {},
            {'step':6, 'degree':HeatCanvas.LINEAR, 'opacity':0.7});
    ...
    for(var i=0,l=data.length; i<l; i++) {
        console.log(data[i].Latitude+"," +data[i].Longitude+","+ data[i].Score);
        heatmap.pushData(data[i].Latitude, data[i].Longitude, data[i].Score);
    }
    console.log(heatmap);
